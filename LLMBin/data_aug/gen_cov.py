...
def gen_bedtools_out(bam_file: str, bam_index: int, out: str, logger):
    ...
    if not os.path.exists(bam_depth):
        ...
        print(f"bam_depth={bam_depth} exists.")
...
def calculate_coverage_samplebyindex(depth_file: str, augpredix: str, aug_seq_info_dict: dict, logger, edge: int = 0,
    ...
    for contig_name, lines in groupby(open(depth_file), lambda ell: ell.split('\t', 1)[0]):
        ...
        if len(depth_value) <= cov_threshold or contig_name not in aug_seq_info_dict.keys():
            continue
...
def gen_cov_from_bedout(logger, out_path: str, depth_file_path: str,
    ...
    for i in range(len(namelist)):
        ...
        if not os.path.exists(depth_file) or not os.path.exists(depth_file + '_aug0_data_cov.csv'):
            pool.apply_async(
                calculate_coverage,
                args=(depth_file, logger, edge, contig_len),
                callback=_checkback)
        else:
            print(f"depth_file={depth_file} exists.")
...
def run_gen_cov(logger, args):
    ...
    depths = glob.glob(os.path.join(args.out_augdata_path, "*_datacoverage_mean.tsv"))
    if len(depths) != args.n_views:
