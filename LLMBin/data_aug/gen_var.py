...
def run_gen_cov_var(logger, args):
    ...
    vars = glob.glob(os.path.join(args.out_augdata_path, "*_datacoverage_var.tsv"))
    if len(vars) != args.n_views:
        gen_cov_var_from_bedout(logger, args.out_augdata_path, out, num_aug=args.n_views-1,
                                contig_len=args.contig_len,num_process=args.num_threads)
