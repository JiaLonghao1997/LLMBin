...
# sys.path.remove("/home1/jialh/metaHiC/tools/BERTBin/BERTBin/data_aug/hyenadna")
sys.path.append("/home1/jialh/metaHiC/tools/BERTBin/BERTBin/data_aug/caduceus")
...
class SequenceLightningModule(pl.LightningModule):
    ...
    def setup(self, stage=None):
        ...
        #     print(f"self.hparams.model.config: {self.hparams.model.config}")
        #     print(f"self.dataset.tokenizer: {self.dataset.tokenizer}")
        #     print(f"self.dataset.tokenizer.complement_map: {self.dataset.tokenizer.complement_map}")
        #     OmegaConf.update(
        ...
        if self.hparams.model.get("config", None) is not None and self.hparams.model.config.get("_target_",
                                                                                                None) is not None:
...
class BatchPredictor:
    ...
    def process_batch(self, sequences: List[str]) -> torch.Tensor:
        ...
        with torch.no_grad():
            ...
            # https://github.com/kuleshov-group/caduceus/blob/main/caduceus/modeling_caduceus.py#L392
            last_hidden_state = self.model.model.forward(input_ids, output_hidden_states=True).hidden_states[-1]
            # print(f"type(last_hidden_state): {type(last_hidden_state)}, last_hidden_state: {last_hidden_state.size()}")
            batch_emb = torch.sum(last_hidden_state, dim=1)
            # print(f"batch_emb: {batch_emb}")
    ...
    def predict_from_fna(self, dna_sequences: list,
        ...
        train_loader = torch.utils.data.DataLoader(dna_sequences, batch_size=batch_size, shuffle=False,
                                                   num_workers=8)
...
def gtdb_caduceus_embedding(dna_sequences, logger, device, model_name_or_path, model_max_length=400, batch_size=20):
    logger.info("**********begin to get embeddings from GTDB+Caduceus*************")
    # OmegaConf.register_new_resolver('eval', eval)
    # OmegaConf.register_new_resolver('div_up', lambda x, y: (x + y - 1) // y)
    # OmegaConf.register_new_resolver('min', lambda x, y: min([x, y]))

    config_path = os.path.join(model_name_or_path, "config_tree.txt")
    checkpoint_path = os.path.join(model_name_or_path, "checkpoints_step30w", "last.ckpt")
    ...
    embedding = predictor.predict_from_fna(dna_sequences=dna_sequences,
                                           batch_size=batch_size)
    logger.info(f"*****************Caduceus embedding.shape={embedding.shape}")
    return embedding