...
def get_kmer_coverage(data_path: str, n_views: int = 2, kmer_model_path: str = 'empty',
                      device = torch.device('cpu'), nokmer: bool = False, model: str = "dnabert2", model_max_length: int = 5000,
                      cov_meannormalize: bool = False,
    ...
    :param model:  Language model for DNA sequence embedding (default: dnabert2).
    :param model_max_length: Max length of Language model (default: 5000).
    ...
    for view in range(n_views):
        ...
        emb_file = data_path + 'aug' + str(view) + f'/aug{view}_{model}_{model_max_length}bp_embedding_sum.tsv'
        # print(f"emb_file in get_augfeatures.py: {emb_file}")
        ...
        covIdxArr = np.empty(len(mapObj), dtype=np.int32)
        ...
        shuffled_embMat = pd.read_csv(emb_file, sep='\t', usecols=range(1, embHeader.shape[1])).values
        shuffled_namelist = pd.read_csv(emb_file, sep='\t', usecols=range(1)).values[:, 0]

        covIdxArr = np.empty(len(mapObj), dtype=np.int32)
        for contigIdx in range(len(shuffled_namelist)):
            if shuffled_namelist[contigIdx].split('_aug')[0] in mapObj:
                covIdxArr[mapObj[shuffled_namelist[contigIdx].split('_aug')[0]]] = contigIdx
        embMat = shuffled_embMat[covIdxArr]
        ...
        if view == 0:
            ...
            embMats = np.vstack((embMats, embMat))
            if addvars:
    ...
    # print("************************Begin to run PCA.*****************")
    # pca = PCA(n_components=100)
    # embMats_new = pca.fit_transform(embMats)
    print(f"covMats.shape={covMats.shape}")
    print(f"compositMats.shape={compositMats.shape}")
    print(f"embMats.shape={embMats.shape}")
    if not nokmer:
        X_ts = np.hstack((covMats, compositMats, embMats))
    else:
        X_ts = np.hstack((covMats, embMats))
...
def get_ContrastiveLearningDataset(data_path: str, n_views: int = 2, kmer_model_path: str = 'empty',
    ...
    if not data_path.endswith('/'):
        data_path = data_path + '/'
    dataset, namelist = get_kmer_coverage(data_path, n_views, kmer_model_path, device, nokmer, model, model_max_length,
                                          cov_meannormalize, cov_minmaxnormalize, cov_standardization,addvars,vars_sqrt,kmer_l2_normalize,kmerMetric_notl2normalize)
