...
def train_CLmodel(logger, args):
    ...
    dataset, namelist = get_ContrastiveLearningDataset(args.data, args.n_views,
                                                       args.kmer_model_path, args.device,
                                                       args.nokmer, args.model, args.model_max_length,
                                                       args.cov_meannormalize,
                                                       args.cov_minmaxnormalize, args.cov_standardization,
                                                       args.addvars, args.vars_sqrt,
                                                       args.kmer_l2_normalize, args.kmerMetric_notl2normalize)
    ...
    if not args.add_model_for_coverage:
        ...
        # 默认情况下pretrain_kmer_model_path: 'no'
        if args.pretrain_kmer_model_path !='no':
            ...
            print("**************begin to train CLmodel from scratch.************")
            # 相关参数见结果目录的config.yml文件，通常args.kmer_model_path=="empty"。
            # 默认情况下out_dim_forcov: 128
            if args.model == "GTDB_Caduceus":
                llm_dim = 512
            elif args.model == "GTDB_HyenaDNA":
                llm_dim = 256
            else:
                llm_dim = None

            if args.kmer_model_path == 'empty':
                cov_dim = len(dataset[0][0]) - 136 - llm_dim
                input_size = args.out_dim_forcov + 136 + args.out_dim_forllm
            else:
                cov_dim = len(dataset[0][0]) - 128 - llm_dim
                input_size = args.out_dim_forcov + 128 + args.out_dim_forllm
                print('cov_dim:\t' + str(cov_dim) + '\n')

            emb_szs_list_forcov = [args.emb_szs_forcov] * args.n_layer_forcov

            # EmbeddingNet类来自/home1/jialh/metaHiC/tools/BERTBin/BERTBin/models/mlp.py
            cov_model = EmbeddingNet(
                in_sz=cov_dim,
                out_sz=args.out_dim_forcov,
                emb_szs=emb_szs_list_forcov,
                ps=[args.dropout_value] * (args.n_layer_forcov - 1),
                use_bn=True,
                actn=nn.LeakyReLU(),
            )

            # 默认情况下pretrain_coveragemodel: false
            ...
            ## 定义针对llm_emb的模型:
            emb_szs_list_forllm = [args.emb_szs_forllm] * args.n_layer_forllm

            # EmbeddingNet类来自/home1/jialh/metaHiC/tools/BERTBin/BERTBin/models/mlp.py
            llm_model = EmbeddingNet(
                in_sz=llm_dim,
                out_sz=args.out_dim_forllm,
                emb_szs=emb_szs_list_forllm,
                ps=[args.dropout_value] * (args.n_layer_forllm - 1),
                use_bn=True,
                actn=nn.LeakyReLU(),
            )
            ...
            model = EmbeddingNet2(
                ...
                covmodel_notl2normalize=args.covmodel_notl2normalize,
                llm_model=llm_model,
                llmmodel_notl2normalize=args.llmmodel_notl2normalize
            )
            optimizer = torch.optim.AdamW(model.parameters(), args.lr, weight_decay=args.weight_decay)
