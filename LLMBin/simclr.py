...
class SimCLR(object):
    ...
    def llmmodel_info_nce_loss(self, features):
        """
        Calculate the InfoNCE loss for the k-mer model.
        ...
        logits = logits / self.args.llmmodel_temperature
        return logits, labels
    ...
    def train(self, train_loader, data, namelist):
        ...
        for epoch_counter in range(self.args.epochs):
            # modified
            for contig_features in tqdm(train_loader, desc=f"Training (epoch={epoch_counter})"):
    ...
    def train_addpretrain(self, train_loader, data, namelist):
        ...
        if self.args.model == 'GTDB_Caduceus':
            llm_emb_size = 512
        elif self.args.model == 'GTDB_HyenaDNA':
            llm_emb_size = 256
        else:
            llm_emb_size = 0

        logging.info(f"data[0].shape: {data[0].shape}")
        covdim = len(data[0][0]) - kmer_len - llm_emb_size

        logging.info(f'kmer_len={kmer_len}, covdim={covdim}, llm_emb_size={llm_emb_size}')
        ...
        for epoch_counter in range(self.args.epochs):
            ...
            for contig_features in tqdm(train_loader, desc=f"Training (epoch={epoch_counter})"):
                ...
                contig_features = contig_features.to(self.args.device)
                with autocast(enabled=self.args.fp16_precision):
                    # 默认参数为addcovloss: false, kmerloss: false
                    if self.args.addcovloss and not self.args.addkmerloss:
                        if self.args.pretrain_kmer_model_path !='no':
                            features, kmeremb, covemb, llmemb = self.model(contig_features[:, covdim:covdim+kmer_len],
                                                                   contig_features[:, :covdim],
                                                                   contig_features[:, covdim+kmer_len:])
                    ...
                    elif self.args.addcovloss and self.args.addkmerloss and self.args.addllmloss:
                        features, kmeremb, covemb, llmemb = self.model(contig_features[:, covdim:covdim+kmer_len],
                                                                   contig_features[:, :covdim],
                                                                   contig_features[:, covdim+kmer_len:])
                        ...
                        logits4, labels4 = self.llmmodel_info_nce_loss(llmemb)
                        loss4 = self.criterion(logits4, labels4)
                        loss = loss1 + self.args.lambdaloss2 * loss2 + self.args.lambdakmerloss2 *  loss3 + self.args.lambdallmloss2 * loss4
                    else:
                        # features, covemb = self.model(contig_features[:, -kmer_len:], contig_features[:, :-kmer_len])
                        # logging.info(f"Don't consider coverage loss and kmer loss.")
                        # logging.info(f"self.args.addcovloss: {self.args.addcovloss}, self.args.addkmerloss: {self.args.addkmerloss}")
                        if self.args.pretrain_kmer_model_path !='no':
                            features, kmeremb, covemb, llmemb= self.model(contig_features[:, covdim:covdim+kmer_len],
                                                                   contig_features[:, :covdim],
                                                                   contig_features[:, covdim+kmer_len:])
            ...
            if self.args.addllmloss:
                if epoch_counter % 20 == 0:
                    logging.debug(f"Epoch: {epoch_counter}\tllm model loss: {loss4}")
        logging.info("Training has finished.")
        # save model checkpoints
        checkpoint_name = 'checkpoint_{:04d}.pth.tar'.format(self.args.epochs)
        save_checkpoint({
        ...
        with torch.no_grad():
            ...
            out = np.concatenate([self.model(data[0][i:i + bs_, covdim:covdim+kmer_len].to(self.args.device),
                                             data[0][i:i + bs_, :covdim].to(self.args.device),
                                             data[0][i:i + bs_, covdim+kmer_len:].to(self.args.device)
                                             )[0].to('cpu').numpy()
                                  for i in range(0, len(data[0]), bs_)], axis=0)
            embeddings_df = pd.DataFrame(out, index=namelist)

            outfile = self.args.output_path + '/embeddings.tsv'
            logging.info(f"Save embeddings to {outfile} with size={embeddings_df.shape}")
            embeddings_df.to_csv(outfile, sep='\t', header=True)

            kmerout = np.concatenate([self.model(data[0][i:i + bs_, covdim:covdim + kmer_len].to(self.args.device),
                                                data[0][i:i + bs_, :covdim].to(self.args.device),
                                                data[0][i:i + bs_, covdim + kmer_len:].to(self.args.device)
                                                )[1].to('cpu').numpy()
                                     for i in range(0, len(data[0]), bs_)], axis=0)
            embeddings_df = pd.DataFrame(kmerout, index=namelist)
            outfile = self.args.output_path + '/kmerembeddings.tsv'
            logging.info(f"Save kmer embeddings to {outfile} with size={embeddings_df.shape}")
            embeddings_df.to_csv(outfile, sep='\t', header=True)

            covout = np.concatenate([self.model(data[0][i:i + bs_, covdim:covdim+kmer_len].to(self.args.device),
                                                data[0][i:i + bs_, :covdim].to(self.args.device),
                                                data[0][i:i + bs_, covdim + kmer_len:].to(self.args.device)
                                                )[2].to('cpu').numpy()
                                     for i in range(0, len(data[0]), bs_)], axis=0)
            ...
            logging.info(f"Save coverage embeddings to {outfile} with size={embeddings_df.shape}")
            embeddings_df.to_csv(outfile, sep='\t', header=True)

            llmout = np.concatenate([self.model(data[0][i:i + bs_, covdim:covdim + kmer_len].to(self.args.device),
                                                data[0][i:i + bs_, :covdim].to(self.args.device),
                                                data[0][i:i + bs_, covdim + kmer_len:].to(self.args.device)
                                                )[3].to('cpu').numpy()
                                     for i in range(0, len(data[0]), bs_)], axis=0)
            embeddings_df = pd.DataFrame(llmout, index=namelist)
            outfile = self.args.output_path + '/llmembeddings.tsv'
            logging.info(f"Save language model embeddings to {outfile} with size={embeddings_df.shape}")
            embeddings_df.to_csv(outfile, sep='\t', header=True)
